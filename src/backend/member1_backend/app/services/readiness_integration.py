...
def run_readiness_pipeline(db: Session) -> dict:
    ...
    recommendations = ReadinessService.rank_maintenance_actions(all_evidence, component_id_map, cost_map)
...
def run_readiness_pipeline(db: Session) -> dict:
    ...
    for ca in cost_assumptions:
        cost_map[ca.component_type] = {
            ...
            "replacement_cost": ca.replacement_cost,
            "failure_impact_cost": ca.failure_impact_cost,
            "emergency_intervention_cost": ca.emergency_intervention_cost,
            "emergency_maintenance_cost": ca.emergency_maintenance_cost,
            "downtime_cost_per_hour": ca.downtime_cost_per_hour,
            "planned_downtime_hours": ca.planned_downtime_hours,
            "emergency_downtime_hours": ca.emergency_downtime_hours,
            "mission_disruption_cost": ca.mission_disruption_cost,
            "assetsentinel_deployment_cost": ca.assetsentinel_deployment_cost,
            "monitoring_cost_traditional": ca.monitoring_cost_traditional,
            "inspection_cost_traditional": ca.inspection_cost_traditional,
            "preventive_maintenance_traditional": ca.preventive_maintenance_traditional,
            "sensor_data_cost_assetsentinel": ca.sensor_data_cost_assetsentinel,
            "residual_failure_probability_multiplier": ca.residual_failure_probability_multiplier
        }
...
def _persist_recommendation(db: Session, rec) -> None:
    ...
    if existing:
        ...
        existing.economic_impact = rec.economic_impact
        existing.timeline = rec.timeline
    else:
